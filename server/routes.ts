...
export async function registerRoutes(
  ...
  app.post("/api/demo/bootstrap", demoAuth, async (req: any, res) => {
    try {
  ...
  app.get("/api/taste-profile", demoAuth, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  app.post("/api/onboarding", demoAuth, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  app.post("/api/onboarding/reset", demoAuth, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  app.post("/api/interactions", demoAuth, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  app.get("/api/social/matches", demoAuth, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  app.get("/api/friends", demoAuth, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  app.get("/api/friends/suggestions", demoAuth, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const friendIds = await storage.getFriendIds(userId);
  ...
  app.post("/api/friends/:friendId", demoAuth, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  app.delete("/api/friends/:friendId", demoAuth, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const { friendId } = req.params;
  ...
  app.get("/api/events", demoAuth, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  app.post("/api/events/:id/rsvp", demoAuth, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  app.get("/api/events/:id/matches", demoAuth, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const eventId = req.params.id;
  ...
  app.get("/api/interactions/collection", demoAuth, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  app.get("/api/explore/hobbies", demoAuth, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
  ...
  app.post("/api/demo/toggle-extreme", demoAuth, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      ...
      const turningOn = !profile.isExtremeDemo;

      if (turningOn) {
        const normalTraits = {
          traitNovelty: profile.traitNovelty,
          traitIntensity: profile.traitIntensity,
          traitCozy: profile.traitCozy,
          traitStrategy: profile.traitStrategy,
          traitSocial: profile.traitSocial,
          traitCreativity: profile.traitCreativity,
          traitNostalgia: profile.traitNostalgia,
          traitAdventure: profile.traitAdventure,
          topClusters: profile.topClusters,
        };
        ...
        const clusters = generateClusters(extremeTraits);

        await storage.upsertTasteProfile({
          userId,
          traitNovelty: extremeTraits.novelty,
          traitIntensity: extremeTraits.intensity,
          traitCozy: extremeTraits.cozy,
          traitStrategy: extremeTraits.strategy,
          traitSocial: extremeTraits.social,
          traitCreativity: extremeTraits.creativity,
          traitNostalgia: extremeTraits.nostalgia,
          traitAdventure: extremeTraits.adventure,
          topClusters: clusters,
          onboardingComplete: true,
          isExtremeDemo: true,
        ...
        const traitText = [
          `Computer Science senior at Texas A&M University, graduating 2026.`,
          `Career goals: AI research, startup founder, software engineering at cutting-edge tech companies.`,
          `Personality: very high novelty (0.90), very high creativity (0.92), very high adventure (0.94), very high strategy (0.88), high intensity (0.78), moderate social (0.62), very low cozy (0.18), very low nostalgia (0.22).`,
          `Clusters: ${clusters.join(", ")}.`,
          `I strongly prefer hackathon competitions, AI and machine learning research, competitive programming, cybersecurity CTF challenges, and robotics projects.`,
          `I strongly prefer startup pitch competitions, product management, cloud computing, systems architecture, and open-source software development.`,
          `I strongly prefer rock climbing, trail running, adventure sports, and outdoor exploration over indoor leisure.`,
          `I avoid purely social gatherings without intellectual substance, passive entertainment, cozy comfort-zone activities, and nostalgic tradition-focused events.`,
          `I avoid low-intensity study groups, casual food-only meetups, and activities without strategic or creative challenge.`,
          `Key interests: hackathon, AI, machine learning, research, robotics, startup, entrepreneurship, cybersecurity, cloud, security, IEEE, consulting, competitive programming, product management, data science, software engineering, innovation, technology.`,
        ].join(" ");
        const profileRow = await pool.query("SELECT id FROM taste_profiles WHERE user_id = $1", [userId]);
        if (profileRow.rows.length > 0) {
          const embedding = await generateEmbedding(traitText);
          await storeEmbedding("taste_profiles", profileRow.rows[0].id, embedding);
        }

        console.log(`[demo] Extreme Persona ON for user ${userId}`);
        return res.json({
          isExtremeDemo: true,
          traits: extremeTraits,
          topClusters: clusters,
        });
      } else {
        const saved = profile.normalTraits as any;
        if (saved) {
          await storage.upsertTasteProfile({
            userId,
            traitNovelty: saved.traitNovelty ?? 0.5,
            traitIntensity: saved.traitIntensity ?? 0.5,
            traitCozy: saved.traitCozy ?? 0.5,
            traitStrategy: saved.traitStrategy ?? 0.5,
            traitSocial: saved.traitSocial ?? 0.5,
            traitCreativity: saved.traitCreativity ?? 0.5,
            traitNostalgia: saved.traitNostalgia ?? 0.5,
            traitAdventure: saved.traitAdventure ?? 0.5,
            topClusters: saved.topClusters ?? [],
            onboardingComplete: true,
            isExtremeDemo: false,
            normalTraits: null,
          } as any);
        ...
        const embResult2 = await recomputeTasteEmbedding(userId);
        if (!embResult2.updated) {
          const restoredProfile = await storage.getTasteProfile(userId);
          if (restoredProfile) {
            const traitText = [
              `Taste profile traits:`,
              `novelty=${restoredProfile.traitNovelty}`,
              `intensity=${restoredProfile.traitIntensity}`,
              `cozy=${restoredProfile.traitCozy}`,
              `strategy=${restoredProfile.traitStrategy}`,
              `social=${restoredProfile.traitSocial}`,
              `creativity=${restoredProfile.traitCreativity}`,
              `nostalgia=${restoredProfile.traitNostalgia}`,
              `adventure=${restoredProfile.traitAdventure}`,
              restoredProfile.topClusters ? `Clusters: ${restoredProfile.topClusters.join(", ")}` : "",
            ].filter(Boolean).join(". ");
            const profileRow = await pool.query("SELECT id FROM taste_profiles WHERE user_id = $1", [userId]);
            if (profileRow.rows.length > 0) {
              const embedding = await generateEmbedding(traitText);
        ...
        const restored = await storage.getTasteProfile(userId);
        console.log(`[demo] Extreme Persona OFF for user ${userId}`);
        return res.json({
          isExtremeDemo: false,
          traits: {
            novelty: restored?.traitNovelty,
            intensity: restored?.traitIntensity,
            cozy: restored?.traitCozy,
            strategy: restored?.traitStrategy,
            social: restored?.traitSocial,
            creativity: restored?.traitCreativity,
            nostalgia: restored?.traitNostalgia,
            adventure: restored?.traitAdventure,
          },
          topClusters: restored?.topClusters,
        });
      }
    } catch (error) {
      console.error("Error toggling extreme persona:", error);
      res.status(500).json({ message: "Failed to toggle extreme persona" });
    }
  });

  app.get("/api/debug/profile-summary", demoAuth, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const profile = await storage.getTasteProfile(userId);
      if (!profile) {
      ...
      const hasEmbedding = profile.embedding != null && Array.isArray(profile.embedding) && profile.embedding.length > 0;

      res.json({
        exists: true,
        userId,
        isExtremeDemo: profile.isExtremeDemo ?? false,
        traits: {
          novelty: profile.traitNovelty,
          intensity: profile.traitIntensity,
          cozy: profile.traitCozy,
          strategy: profile.traitStrategy,
          social: profile.traitSocial,
          creativity: profile.traitCreativity,
          nostalgia: profile.traitNostalgia,
          adventure: profile.traitAdventure,
        },
        topClusters: profile.topClusters,
        onboardingComplete: profile.onboardingComplete,
        tasteEmbedding: {
          exists: hasEmbedding,
          dimension: hasEmbedding ? (profile.embedding as number[]).length : 0,
          updatedAt: profile.embeddingUpdatedAt,
        },
      });
    } catch (error) {
    ...
    try {
      const counts = await pool.query(`
        SELECT
          ...
          (SELECT count(*) FROM event_rsvps) as rsvps,
          (SELECT count(*) FROM friendships) as friendships,
          (SELECT count(*) FROM taste_profiles) as profiles,
          (SELECT count(*) FROM interactions) as interactions,
          (SELECT count(*) FROM hobbies) as hobbies
      `);
      ...
      res.json({
        ...
        replitDeployment: process.env.REPLIT_DEPLOYMENT || "none",
        dbHost,
        ...
        counts: {
          ...
          friendships: Number(row.friendships),
          profiles: Number(row.profiles),
          interactions: Number(row.interactions),
          hobbies: Number(row.hobbies),
        },
      });
    } catch (error) {
      console.error("Error fetching runtime info:", error);
      res.status(500).json({ message: "Failed to fetch runtime info" });
    }
  });

  app.get("/api/debug/parity-report", async (_req, res) => {
    try {
      const dbUrl = process.env.DATABASE_URL || "";
      const hostMatch = dbUrl.match(/@([^:/]+)/);
      const dbHost = hostMatch ? hostMatch[1] : "unknown";
      ...
      const dbInfo = await pool.query(`
        SELECT current_database() as db_name, inet_server_addr() as server_addr, inet_server_port() as server_port, version() as pg_version
      `);
      ...
      const sampleUsers = await pool.query("SELECT id, first_name, last_name FROM users ORDER BY id LIMIT 3");
      const sampleEvents = await pool.query("SELECT id, title FROM events ORDER BY id LIMIT 3");
      const sampleItems = await pool.query("SELECT id, title, image_url FROM items ORDER BY id LIMIT 3");

      const demoUserCheck = await pool.query("SELECT id, first_name, last_name FROM users WHERE id = '49486139'");
      const demoProfileCheck = await pool.query("SELECT user_id, onboarding_complete FROM taste_profiles WHERE user_id = '49486139'");
      const demoFriendCount = await pool.query("SELECT count(*) as count FROM friendships WHERE user_id = '49486139'");
      const demoRsvpCount = await pool.query("SELECT count(*) as count FROM event_rsvps WHERE user_id = '49486139'");
      const demoInteractionCount = await pool.query("SELECT count(*) as count FROM interactions WHERE user_id = '49486139'");

      const andyCheck = await pool.query("SELECT id, first_name, last_name FROM users WHERE first_name = 'Andrew' AND last_name = 'Park'");

      const missingEmbeddings = await pool.query(`
        SELECT
          (SELECT count(*) FROM items WHERE embedding IS NULL) as items_missing,
          (SELECT count(*) FROM events WHERE embedding IS NULL) as events_missing,
          (SELECT count(*) FROM hobbies WHERE embedding IS NULL) as hobbies_missing,
          (SELECT count(*) FROM taste_profiles WHERE embedding IS NULL AND onboarding_complete = true) as profiles_missing
      `);
      const emb = missingEmbeddings.rows[0];
      ...
      if (fs.existsSync(distPath)) {
        assetsList = fs.readdirSync(distPath).slice(0, 10);
      }
      ...
      res.json({
        ...
        nodeEnv: process.env.NODE_ENV || "development",
        isPublished: process.env.REPLIT_DEPLOYMENT === "1" || process.env.NODE_ENV === "production",
        demoModeFlags: {
          DEMO_BYPASS_AUTH: process.env.DEMO_BYPASS_AUTH || "not set",
        },
        databaseFingerprint: {
          host: dbHost,
          dbName: dbInfo.rows[0]?.db_name,
          serverAddr: dbInfo.rows[0]?.server_addr,
          serverPort: dbInfo.rows[0]?.server_port,
          pgVersion: dbInfo.rows[0]?.pg_version?.substring(0, 60),
        },
        seedFingerprint: {
          counts: {
            ...
            rsvps: Number(c.rsvps),
            friendships: Number(c.friendships),
            profiles: Number(c.profiles),
            interactions: Number(c.interactions),
            hobbies: Number(c.hobbies),
          },
          sampleUsers: sampleUsers.rows,
          sampleEvents: sampleEvents.rows,
          sampleItems: sampleItems.rows.map((r: any) => ({ id: r.id, title: r.title, imageUrl: r.image_url })),
          hasAndyChen: andyCheck.rows.length > 0,
          hasAndyPark: andyCheck.rows.length > 0,
        },
        demoUser: {
          exists: demoUserCheck.rows.length > 0,
          name: demoUserCheck.rows[0] ? `${demoUserCheck.rows[0].first_name} ${demoUserCheck.rows[0].last_name}` : null,
          hasProfile: demoProfileCheck.rows.length > 0 && demoProfileCheck.rows[0]?.onboarding_complete,
          friendCount: Number(demoFriendCount.rows[0]?.count || 0),
          rsvpCount: Number(demoRsvpCount.rows[0]?.count || 0),
          interactionCount: Number(demoInteractionCount.rows[0]?.count || 0),
        },
        embeddingHealth: {
          itemsMissing: Number(emb.items_missing),
          eventsMissing: Number(emb.events_missing),
          hobbiesMissing: Number(emb.hobbies_missing),
          profilesMissing: Number(emb.profiles_missing),
        },
        assetsServing: {
          distPublicExists,
          servingFrom: distPublicPath,
          firstAssets: assetsList,
        },
      });
    } catch (error) {
      console.error("Error generating parity report:", error);
      res.status(500).json({ message: "Failed to generate parity report", error: String(error) });
    }
  });

  app.get("/api/debug/score-distribution", demoAuth, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const domain = (req.query.domain as string) || "academic";
      const profile = await storage.getTasteProfile(userId);
      if (!profile || !isValidEmbedding(profile.embedding)) {
        return res.json({ error: "No profile or embedding found" });
      }

      const targetTable = domain === "events" ? "events" : "items";
      const queryStr = domain === "events"
        ? `SELECT id, title, embedding FROM events WHERE embedding IS NOT NULL`
        : `SELECT id, title, embedding FROM items WHERE embedding IS NOT NULL AND domain = $1`;
      const params = domain === "events" ? [] : [domain];
      const itemsResult = await pool.query(queryStr, params);

      const scored = itemsResult.rows.map((row: any) => {
        let emb: number[];
        ...
        const cosSim = computeCosineSimilarity(profile.embedding!, emb);
        const vectorScore = cosineSimilarityToScore(cosSim);
        return { id: row.id, title: row.title, cosineSimilarity: Math.round(cosSim * 10000) / 10000, vectorScore };
      });

      scored.sort((a: any, b: any) => b.cosineSimilarity - a.cosineSimilarity);

      const cosValues = scored.map((s: any) => s.cosineSimilarity);
      const scoreValues = scored.map((s: any) => s.vectorScore);
      ...
      res.json({
        ...
        cosineSimilarity: {
          min: Math.min(...cosValues),
          median: Math.round(median(cosValues) * 10000) / 10000,
          max: Math.max(...cosValues),
        },
        vectorScore: {
        ...
        weights: profile.isExtremeDemo ? { vector: 0.70, cf: 0.20, traits: 0.10 } : { vector: 0.55, cf: 0.25, traits: 0.20 },
      });
    } catch (error) {
      console.error("Error computing score distribution:", error);
      res.status(500).json({ message: "Failed to compute score distribution" });
    }
  });
