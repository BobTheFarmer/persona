...
export function buildEmbeddingText(item: {
  ...
  if (item.meetingLocation || item.location) {
    parts.push(`Location: ${item.meetingLocation || item.location}`);
  }
  if (item.meetingDay && item.meetingTime) {
    parts.push(`Meets: ${item.meetingDay} at ${item.meetingTime}`);
  }
  if (item.organizerName) {
    parts.push(`Organizer: ${item.organizerName}`);
  }
  if (item.priceInfo) {
    parts.push(`Price: ${item.priceInfo}`);
  }
  return parts.join(". ");
}
...
export function cosineSimilarityToScore(similarity: number): number {
  const score = 100 / (1 + Math.exp(-(similarity - 0.25) * 8));
