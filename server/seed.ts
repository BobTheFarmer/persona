...
async function seedEmbeddings() {
  const missingItemEmbeddings = await pool.query(
    "SELECT id, title, tags, description, domain, meeting_day, meeting_time, meeting_location FROM items WHERE embedding IS NULL"
  );
  ...
  const missingEventEmbeddings = await pool.query(
    "SELECT id, title, tags, description, location, category, price_info, organizer_name FROM events WHERE embedding IS NULL"
  );
...
export async function seedDatabase() {
  ...
  const DEMO_USER_ID = "49486139";
  const [demoUserCheck] = await db.select().from(users).where(eq(users.id, DEMO_USER_ID));
  if (!demoUserCheck) {
    console.log("Seeding demo user (Garv Shah)...");
    const allClubs = await db.select().from(items);
    const aimlClub = allClubs.find(c => c.title === "TAMU AI/ML Club");
    await db.insert(users).values({
      ...
      mainClubItemId: aimlClub?.id || null,
    });
    await storage.upsertTasteProfile({ userId: DEMO_USER_ID, ...GARV_PROFILE });
    console.log("  Demo user created with GARV_PROFILE.");
  } else {
    const existingProfile = await storage.getTasteProfile(DEMO_USER_ID);
    if (!existingProfile || !existingProfile.onboardingComplete) {
      await storage.upsertTasteProfile({ userId: DEMO_USER_ID, ...GARV_PROFILE });
      console.log("  Demo user profile restored with GARV_PROFILE.");
    }
  }
  ...
  const [demoFriendCheck] = await db.select({ count: sql<number>`count(*)` }).from(friendships).where(sql`user_id = ${DEMO_USER_ID}`);
  if (Number(demoFriendCheck.count) === 0) {
    console.log("Seeding demo user friendships...");
    const demoFriends = ["seed-colin", "seed-andy", "seed-maya", "seed-jake", "seed-sophia", "seed-devon", "seed-ravi", "seed-emma"];
    for (const friendId of demoFriends) {
      try {
        await db.insert(friendships).values({ userId: DEMO_USER_ID, friendId });
        await db.insert(friendships).values({ userId: friendId, friendId: DEMO_USER_ID });
      } catch (e) {}
    }
    console.log("Demo user friendships seeded.");
  }

  const [demoRsvpCheck] = await db.select({ count: sql<number>`count(*)` }).from(eventRsvps).where(sql`user_id = ${DEMO_USER_ID}`);
  if (Number(demoRsvpCheck.count) === 0) {
    console.log("Seeding demo user RSVPs...");
    const allEvts = await db.select().from(events);
    const demoEventIndices = [0, 2, 5, 7, 10, 14, 18, 22];
    for (const idx of demoEventIndices) {
      if (idx < allEvts.length) {
        try {
          await storage.createEventRsvp({ eventId: allEvts[idx].id, userId: DEMO_USER_ID });
        } catch (e) {}
      }
    }
    console.log("Demo user RSVPs seeded.");
  }
  ...
  if (Number(interactionCheck.count) === 0) {
    ...
    const seedInteractions: { userId: string; clubs: string[]; action: string }[] = [
      ...
      { userId: DEMO_USER_ID, clubs: ["TAMU AI/ML Club", "Aggie Coding Club", "TAMU Entrepreneurship Club", "Undergraduate Research Scholars", "Data Science Club"], action: "like" },
    ];
